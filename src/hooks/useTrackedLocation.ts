...
export function useTrackedLocation(options: UseTrackedLocationOptions = {}) {
  ...
  useEffect(() => {
    if (enableTracking && userId && geolocation.latitude && geolocation.longitude) {
      syncLocation();
    }
  }, [enableTracking, userId, geolocation.latitude, geolocation.longitude, syncLocation]);
  ...
  }, [enableTracking, userId, trackingInterval, syncLocation, geolocation.latitude, geolocation.longitude]);
