...
export default function CRMPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/customers?action=stats&companyId=${user.companyId}`
      );
      const data = await response.json();
      if (data.success) {
        setStats(data.stats);
  ...
  const fetchTags = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/customers?action=tags&companyId=${user.companyId}`
      );
      const data = await response.json();
      if (data.success) {
  ...
  const fetchCustomerDetails = async (customerId: string) => {
    try {
      const [interactionsRes, notesRes, purchasesRes] = await Promise.all([
        fetch(`/api/customers/interactions?customerId=${customerId}`),
        fetch(`/api/customers/notes?customerId=${customerId}`),
        fetch(`/api/customers/purchases?customerId=${customerId}`),
      ]);
  ...
  const handleAddInteraction = async () => {
    ...
    try {
      const response = await fetch("/api/customers/interactions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
